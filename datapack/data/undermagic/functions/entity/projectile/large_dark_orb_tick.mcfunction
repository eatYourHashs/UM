...
execute if score @s um.dummy matches 5.. run scoreboard players set $math.in_0 um.dummy 190
...
execute if score @s um.dummy matches 5.. run scoreboard players set @e[distance=..2,tag=!global.ignore] um.aw_mark 60
execute if entity @a[distance=..3,scores={um.kinet_charms=1..}] if score @s um.dummy matches 5.. unless entity @s[tag=um.kinet_proc] run function undermagic:player/charms/kinetic_field_strong