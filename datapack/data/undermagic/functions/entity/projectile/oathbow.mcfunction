...
execute as @e[tag=!global.ignore,distance=..2,scores={um.oathbow_mark=1..}] run function undermagic:utils/damage_entity
scoreboard players set $math.in_0 um.dummy 100
scoreboard players set $math.in_1 um.dummy 0
scoreboard players set $math.in_2 um.dummy 0
execute as @e[tag=!global.ignore,distance=..2,scores={um.oathbow_mark=..0}] run function undermagic:utils/damage_entity
